...
export function setupTpLogStreaming(server: Server, processManager: TpProcessManager) {
  ...
  wss.on('connection', (ws: WebSocket) => {
    logger.info('WebSocket client connected for TP logs');
    ...
    ws.on('close', () => {
      logger.info('WebSocket client disconnected for TP logs');
  ...
  logger.info('WebSocket TP log streaming setup complete');
  return wss;
}
