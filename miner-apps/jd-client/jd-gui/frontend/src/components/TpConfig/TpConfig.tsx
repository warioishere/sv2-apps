...
export function TpConfig() {
  ...
  // Poll Bitcoin Core network and auto-sync TP config when it changes
  useEffect(() => {
    const pollBitcoinNetwork = async () => {
      try {
        const response = await fetch('/api/bitcoin/status');
        const bitcoinStatus = await response.json();

        if (!bitcoinStatus.running || !bitcoinStatus.network) return;
        ...
        // On first poll, just record the network and set initial config
        if (lastKnownBitcoinNetwork.current === null) {
          lastKnownBitcoinNetwork.current = detectedNetwork;
        ...
        // If network hasn't changed, nothing to do
        if (lastKnownBitcoinNetwork.current === detectedNetwork) return;

        // Network changed — update the ref
        lastKnownBitcoinNetwork.current = detectedNetwork;

        // Skip auto-switch if user chose custom path
        if (configRef.current.bitcoin_source === 'host-custom') return;

        // Skip auto-switch if TP is currently running
        if (statusRef.current.running) return;

        // Build the updated config using current form values + new network
        const currentConfig = configRef.current;
        const newConfig = {
          ...currentConfig,
          bitcoin_source: bitcoinSource as TpConfigData['bitcoin_source'],
          network: detectedNetwork as TpConfigData['network'],
        };
        setConfig(newConfig);

        // Auto-save: generate config inline for the integrated source path
        const dataDir = detectedNetwork === 'mainnet' ? '/bitcoin-ipc-mainnet' : '/bitcoin-ipc-testnet';
        const networkFlag = detectedNetwork === 'mainnet' ? '' : 'testnet4=1';
        const configStr = `# Stratum V2 Template Provider Configuration
...
sv2bind=${currentConfig.listen_address}

# Stratum V2 interval (seconds) - how often to check for new templates
sv2interval=${currentConfig.fee_check_interval}

# Fee delta (sats/vB) - minimum fee rate threshold
sv2feedelta=${currentConfig.min_fee_rate}

# Logging (printtoconsole avoids needing writable datadir for sv2-debug.log)
printtoconsole=1
debug=sv2
loglevel=sv2:${currentConfig.log_level}
debug=ipc
`;

        try {
          const saveResponse = await fetch('/api/tp/config', {
            method: 'POST',
          ...
          if (saveResult.success) {
            setMessage({
              type: 'success',
              text: `Bitcoin Core switched to ${detectedNetwork} — TP config updated automatically`,
            });
          }
        } catch (saveError) {
          console.error('Failed to auto-save TP config after network change:', saveError);
        }
      } catch (error) {
    ...
    pollBitcoinNetwork();
    const interval = setInterval(pollBitcoinNetwork, 5000);
  ...
  const parseConfigFromRaw = (rawConfig: string): Partial<TpConfigData> => {
    ...
    for (const line of lines) {
      ...
      if (key === 'datadir') {
        ...
        parsed.network = value === 'test' ? 'testnet' : value;
      } else if (key === 'testnet4' && value === '1') {
        parsed.bitcoin_source = parsed.bitcoin_source || 'integrated-testnet';
        parsed.network = 'testnet';
      } else if (key === 'signet' && value === '1') {
        parsed.network = 'signet';
      } else if (key === 'regtest' && value === '1') {
        parsed.network = 'regtest';
      } else if (key === 'sv2bind') {
  ...
  const generateBitcoinCoreConfig = (): string => {
    ...
    switch (config.bitcoin_source) {
      ...
      case 'integrated-testnet':
        ...
        networkFlag = 'testnet4=1';
        break;
      case 'host-custom':
        bitcoinDataDir = config.custom_path || '/host-bitcoin/mainnet';
        networkFlag = config.network === 'mainnet' ? '' : config.network === 'testnet' ? 'testnet4=1' : `${config.network}=1`;
        break;
      default:
        bitcoinDataDir = '/bitcoin-ipc-testnet';
        networkFlag = 'testnet4=1';
